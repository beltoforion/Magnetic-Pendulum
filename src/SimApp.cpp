...
BOOL CSimApp::InitInstance()
{
  ...
  try
  {
    std::string sFile( CWinApp::m_lpCmdLine );
    if (sFile.length()==0)
      throw std::runtime_error("usage:  SimPend config.cfg\n\nplease provide a config file.");
    ...
    char szName[2048], szDrive[2048], szDir[2048];
    _splitpath(sFile.c_str(), szDrive, szDir, szName, NULL);
    std::stringstream ss;
