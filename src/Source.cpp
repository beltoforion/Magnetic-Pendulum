...
ISource* ISource::CreateSource(const std::string &sType, const ISource::SData &src)
{
  if (sType=="LINEAR")
    return new SourceLin(src);
  else if (sType=="INV")
    return new SourceInv(src);
  else if (sType=="INV_SQR")
    return new SourceInvSqr(src);
  else
  {
    std::stringstream msg;
    msg << "Invalid source type identifier: \"" << sType << "\n";
    msg << "Valid values are \"LINEAR\" or \"INV\" or \"INV_SQUARE\".";
