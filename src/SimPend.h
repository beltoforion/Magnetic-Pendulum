...
public:
  ...
  void Restore( const std::string &sPath, 
                const std::string &sName );
  int Calc( const mu::vec2d_type &start_pos, 
            const mu::vec2d_type &start_vel = mu::vec2d_type(),
            trace_buf_type *pvTrace = NULL);
  const ISource* GetMagnet(std::size_t idx) const;

  void DumpToFile(const std::string &sPath, const std::string &sFile);
  void CreateBitmap(const std::string &sFile);
...
private:
  ...
  parser_handle m_hParser;        ///< Function parser for the color scaling functions
  ...
  ISource* ReadSourceData(const std::string &sSection, const au::IniFile &iniFile);
};
